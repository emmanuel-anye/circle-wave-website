...
export default function RecruitmentOperationsClient({
  applications,
  employers,
  talent,
  activity,
  ...
  const filteredApplications = useMemo(
    ...
    [applicationRows, normalizedQuery, statusFilter]
  );

  const filteredEmployers = useMemo(
    () =>
      employerRows.filter((item) => {
        const text = [
          item.company_name,
          item.contact_name,
          item.email,
    ...
    [employerRows, normalizedQuery, statusFilter]
  );

  const filteredTalent = useMemo(
    () =>
      talentRows.filter((item) => {
        const text = [
          item.full_name,
          item.email,
          item.location,
    ...
    [talentRows, normalizedQuery, statusFilter]
  );
  ...
  function updateRow(
    entity: RecruitmentEntityType,
    id: string,
    status: string,
    internal_notes: string
  ) {
    const updater = <T extends RecordBase>(rows: T[]) =>
      rows.map((item) =>
        item.id === id ? { ...item, status, internal_notes } : item
      );
  ...
  function exportVisibleRows() {
    ...
    if (tab === "employers") {
      downloadCsv(
        "circle-wave-employer-opportunities.csv",
        filteredEmployers.map((item) => ({
          company: item.company_name,
          contact_name: item.contact_name,
          email: item.email,
          roles: item.job_roles,
          hiring_timeline: item.hiring_timeline,
          status: item.status,
          received_at: item.created_at,
          status_updated_at: item.status_updated_at,
        }))
    ...
    downloadCsv(
      "circle-wave-talent-network.csv",
    ...
    <div className="mt-10">
      ...
      <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap gap-3">
          {(["applications", "employers", "talent", "activity"] as Tab[]).map(
            (item) => (
              <button
                key={item}
                type="button"
        ...
        {tab !== "activity" && (
          <div className="mt-5 grid gap-4 md:grid-cols-[1fr_220px_auto]">
            <input
              ...
              placeholder="Search names, email, role, company, or reference"
            ...
            <button
              ...
              className="rounded-xl border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-800 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
            >
              Export CSV
            </button>
          </div>
      ...
      <div className="mt-8 grid gap-5">
        {tab === "applications" &&
          filteredApplications.map((item) => (
            <PipelineCard
              ...
              record={item}
              title={valueOrDash(item.full_name)}
              subtitle={valueOrDash(item.application_reference)}
              statuses={applicationStatuses}
              onSaved={(id, status, notes) =>
                updateRow("job_application", id, status, notes)
              }
              details={[
                ["Email", item.email],
                ["Role", item.job_title_snapshot],
                ...
                ["Status updated", formatDate(item.status_updated_at)],
              ]}
            />
          ))}
        {tab === "employers" &&
          filteredEmployers.map((item) => (
            <PipelineCard
              key={item.id}
              entityType="employer_request"
              record={item}
              title={valueOrDash(item.company_name)}
              subtitle={valueOrDash(item.contact_name)}
              statuses={employerStatuses}
              onSaved={(id, status, notes) =>
                updateRow("employer_request", id, status, notes)
              }
              details={[
                ["Email", item.email],
                ["Roles", item.job_roles],
        ...
        {tab === "talent" &&
          filteredTalent.map((item) => (
            <PipelineCard
              key={item.id}
              entityType="talent_network"
              record={item}
              title={valueOrDash(item.full_name)}
              subtitle={valueOrDash(item.registration_reference)}
              statuses={talentStatuses}
              onSaved={(id, status, notes) =>
                updateRow("talent_network", id, status, notes)
              }
              details={[
                ["Email", item.email],
                ["Target roles", item.target_roles],
                ["Location", item.location],
                ["Received", formatDate(item.created_at)],
                ["Status updated", formatDate(item.status_updated_at)],
              ]}
          ...
          activity.map((item) => (
            ...
            >
              ...
              <p className="mt-2 text-sm text-slate-600">
                {humanizeStatus(item.entity_type)} · {item.entity_id}
              </p>
              {item.action === "status_changed" ? (
                <p className="mt-3 text-sm text-slate-700">
                  {humanizeStatus(item.previous_value)} →{" "}
                  {humanizeStatus(item.new_value)}
                </p>
              ) : (
                <p className="mt-3 text-sm text-slate-700">
                  {item.note || "Notes updated"}
                </p>
              )}
            </article>
          ))}
